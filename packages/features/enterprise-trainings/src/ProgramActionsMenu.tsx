...
import { deleteProgram, duplicateProgram, ProgramsApiError, restoreProgram, updateProgramStatus, type Program, type ProgramListItem } from "./programs.service";
import { canDeleteProgram, canEditProgram, getProgramStatusActions, type ProgramStatusAction } from "./program-status";

/** Props for the shared Program card/detail contextual action menu. */
export interface ProgramActionsMenuProps {
  program: ProgramListItem;
  includeNavigation?: boolean;
  onStatusSuccess?: (program: Program, action: ProgramStatusAction) => void;
  onDuplicateSuccess?: (program: Program) => void;
  onDeleteSuccess?: () => void;
}

interface SecondaryProgramAction { label: string; confirmationTitle: string; confirmationDescription: string; confirmLabel: string; cancelLabel: string; pendingLabel: string; }
type ConfirmationAction = { kind: "status"; action: ProgramStatusAction } | { kind: "duplicate" | "delete"; action: SecondaryProgramAction };
type MenuPosition = { top: number; left: number };

const duplicateProgramAction: SecondaryProgramAction = { label: "Duplicate program", confirmationTitle: "Duplicate program?", confirmationDescription: "A new program will be created using the details from this program.", confirmLabel: "Duplicate program", cancelLabel: "Cancel", pendingLabel: "Duplicating..." };
const deleteProgramAction: SecondaryProgramAction = { label: "Delete program", confirmationTitle: "Delete program?", confirmationDescription: "This program will be removed from your Programs list. This action may not be reversible.", confirmLabel: "Delete program", cancelLabel: "Keep program", pendingLabel: "Deleting..." };

/** Renders accessible Program navigation and backend-authoritative lifecycle actions. */
export default function ProgramActionsMenu({ program, includeNavigation = true, onStatusSuccess, onDuplicateSuccess, onDeleteSuccess }: ProgramActionsMenuProps) {
  ...
  const statusActions = getProgramStatusActions(program.status);
  const canEdit = canEditProgram(program.status);
  const canDelete = canDeleteProgram(program.status);
  ...
  const statusMutation = useMutation({
    mutationFn: (action: ProgramStatusAction) => {
      if (action.targetStatus === "draft" && program.status === "archived") return restoreProgram(program.id) as Promise<Program>;
      return updateProgramStatus(program.id, { status: action.targetStatus, ...(action.targetStatus === "cancelled" && cancellationReason.trim() ? { reason: cancellationReason.trim() } : {}) });
    },
    onSuccess: async (updatedProgram, action) => { await invalidateProgramQueries(queryClient, program.id); closeConfirmation(); onStatusSuccess?.(updatedProgram, action); },
    onError: (error) => { closeConfirmation(); setActionError(getStatusErrorMessage(error)); },
  });
  const duplicateMutation = useMutation({
    mutationFn: () => duplicateProgram(program.id),
    onSuccess: async (duplicatedProgram) => { await queryClient.invalidateQueries({ queryKey: ["programs", "list"] }); closeConfirmation(); onDuplicateSuccess?.(duplicatedProgram); },
    onError: (error) => { closeConfirmation(); setActionError(getDuplicateErrorMessage(error)); },
  });
  const deleteMutation = useMutation({
    mutationFn: () => deleteProgram(program.id),
    onSuccess: async () => { await invalidateProgramQueries(queryClient, program.id); closeConfirmation(); onDeleteSuccess?.(); },
...
function ProgramActionConfirmationDialog({ action, cancellationReason, isPending, cancelButtonRef, onCancellationReasonChange, onCancel, onConfirm, destructive }: { action: ProgramStatusAction | SecondaryProgramAction; cancellationReason: string; isPending: boolean; cancelButtonRef: React.RefObject<HTMLButtonElement | null>; onCancellationReasonChange: (reason: string) => void; onCancel: () => void; onConfirm: () => void; destructive: boolean }) {
  const isCancellation = "targetStatus" in action && action.targetStatus === "cancelled";
  return <div className="fixed inset-0 z-[110] flex items-end bg-[#06201c]/35 p-0 sm:items-center sm:justify-center sm:p-5" role="presentation"><div role="dialog" aria-modal="true" aria-labelledby="program-status-confirmation-title" aria-describedby="program-status-confirmation-description" onKeyDown={trapDialogFocus} className="w-full rounded-t-2xl bg-white p-5 shadow-2xl sm:max-w-md sm:rounded-2xl"><h2 id="program-status-confirmation-title" className="text-lg font-bold text-[#06201c]">{action.confirmationTitle}</h2><p id="program-status-confirmation-description" className="mt-2 text-sm leading-6 text-[#52736a]">{action.confirmationDescription}</p>{isCancellation ? <label className="mt-4 block text-sm font-semibold text-[#06201c]">Reason <span className="font-normal text-[#52736a]">(optional)</span><textarea value={cancellationReason} onChange={(event) => onCancellationReasonChange(event.target.value)} rows={3} className="mt-1 w-full rounded-xl border border-[#d7e5df] p-3 font-normal outline-none focus:border-[#1f6a58] focus:ring-2 focus:ring-[#1f6a58]/20" /></label> : null}<div className="mt-6 flex justify-end gap-3"><button ref={cancelButtonRef} type="button" onClick={onCancel} disabled={isPending} className="h-10 rounded-full border border-[#d7e5df] px-4 text-sm font-semibold text-[#52736a] disabled:opacity-60">{action.cancelLabel}</button><button type="button" onClick={onConfirm} disabled={isPending} className={`h-10 rounded-full px-4 text-sm font-bold text-white disabled:opacity-60 ${destructive ? "bg-[#b42318]" : "bg-[#1f6a58]"}`}>{isPending ? action.pendingLabel : action.confirmLabel}</button></div></div></div>;
}

async function invalidateProgramQueries(queryClient: QueryClient, programId: string) { await Promise.all([queryClient.invalidateQueries({ queryKey: ["programs", "list"] }), queryClient.invalidateQueries({ queryKey: ["programs", "detail", programId] })]); }
function getStatusErrorMessage(error: unknown): string { if (!(error instanceof ProgramsApiError)) return "Unable to update the program status. Please try again."; if (error.status === 401 || error.status === 403) return "You do not have permission to change this program status."; if (error.status === 404) return "This program no longer exists."; if (error.status === 409) return "This status change is not allowed for the program's current lifecycle state."; if (error.status === 422) return error.fieldErrors.status?.[0] ?? "This status change was not accepted."; return "Unable to update the program status. Please try again."; }
function getDuplicateErrorMessage(error: unknown): string { if (!(error instanceof ProgramsApiError)) return "Unable to duplicate this program. Please try again."; if (error.status === 401 || error.status === 403) return "You do not have permission to duplicate this program."; if (error.status === 404) return "This program no longer exists."; return "Unable to duplicate this program. Please try again."; }
function getDeleteErrorMessage(error: unknown): string { if (!(error instanceof ProgramsApiError)) return "Unable to delete this program. Please try again."; if (error.status === 401 || error.status === 403) return "You do not have permission to delete this program."; if (error.status === 404) return "This program no longer exists."; if (error.status === 409) return "This program cannot be deleted because it has related records or lifecycle restrictions."; return "Unable to delete this program. Please try again."; }
