...
/** Deletes one canonical-UUID Super Admin through the server-only session boundary. */
export async function POST(_: Request, { params }: SuperAdminActionRouteContext) {
  ...
  try {
    return superAdminJsonResponse(await requestSuperAdminJson(`/super-admins/${encodeURIComponent(userId)}/delete`, { method: "POST" }));
  } catch (error) {
