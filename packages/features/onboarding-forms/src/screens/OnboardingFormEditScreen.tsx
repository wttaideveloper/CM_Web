...
import SortableSectionCard from "../components/builder/SortableSectionCard";
...
function sortSections(sections: FormSection[]) {
  return [...sections]
...
export default function OnboardingFormEditPage() {
  ...
  useEffect(() => {
    ...
    async function fetchForm() {
      if (!formId) {
        setLoadError("Missing onboarding form id.");
        setIsLoading(false);
        return;
      }
      ...
      try {
        ...
        const hydratedSections = sortSections(hydrateSections(data.sections));

        setForm({
          name: data.name,
          description: data.description,
          entity_type: "enterprise",
          enterprise_type: data.enterprise_type ?? "Healthcare",
          registration_type: data.registration_type ?? "enterprise",
          status: data.status,
          sections: hydratedSections,
        });
        setEnterpriseType(toEnterpriseTypeOption(data.enterprise_type));
        setRegistrationType(toRegistrationTypeLabel(data.registration_type));
        setActiveSectionId(hydratedSections[0]?.id ?? "");
      } catch (fetchError) {
        if (!cancelled) {
          setLoadError(fetchError instanceof Error ? fetchError.message : "Unable to load form.");
        }
      } finally {
        if (!cancelled) {
          setIsLoading(false);
    ...
    return (
      <>
        <div className="rounded-2xl border border-[#f3d0cb] bg-[#fff6f5] p-5 shadow-sm">
          ...
          <p className="mt-2 text-sm text-[#7a271a]">{loadError}</p>
          <div className="mt-4 flex gap-3">
            <button
              type="button"
              onClick={() => router.push("/onboarding-forms")}
  ...
  function validateFormForSave() {
    ...
    const emptySection = form.sections.find((section) => !section.title.trim());
    if (emptySection) {
      setNotice("Section titles cannot be empty.");
      return false;
    }
  ...
  async function saveChanges() {
    ...
    try {
      const response = await updateOnboardingForm(
        formId,
        buildFormPayload(form, form.status, enterpriseType, registrationType),
      );
      ...
      router.push(`/onboarding-forms/${formId}`);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Failed to update form.");
    } finally {
      setIsSaving(false);
    }
  }
  ...
  async function publishForm() {
    ...
    try {
      const response = await updateOnboardingForm(
        formId,
        buildFormPayload(form, "draft", enterpriseType, registrationType),
      );
      setEnterpriseType(toEnterpriseTypeOption(response.enterprise_type));
      setRegistrationType(toRegistrationTypeLabel(response.registration_type));
      setForm((current) => ({
      ...
      await publishOnboardingForm(formId);
  ...
  async function unpublishForm() {
    ...
    try {
      await unpublishOnboardingForm(formId);
      setForm((current) => ({ ...current, status: "draft" }));
      setNotice("Form unpublished.");
      router.push(`/onboarding-forms/${formId}`);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Failed to unpublish form.");
    } finally {
      setIsPublishing(false);
    }
  }
