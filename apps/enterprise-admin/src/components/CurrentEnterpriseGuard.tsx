  ...
  if (!enterpriseId || !currentEnterprise) {
    return (
      <section className="rounded-2xl border border-[#e1ebe6] bg-white px-5 py-16 text-center shadow-sm">
        ...
        <p className="mt-2 text-sm text-[#52736a]">{enterpriseError ?? "No enterprise is linked to this organization yet."}</p>
      </section>
    );
  }

  const enterpriseName =
    currentEnterprise.business_legal_name ||
    currentEnterprise.business_short_name ||
