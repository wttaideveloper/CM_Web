...
export function isProgramConfigurationId(value: string): boolean {
  return configurationIdPattern.test(value);
}

export function invalidProgramConfigurationIdResponse(): NextResponse {
  return NextResponse.json(
    { detail: "A valid program form configuration identifier is required." },
    { status: 400, headers: { "Cache-Control": "no-store" } },
  );
}

export async function proxyProgramFormConfigurationGet(pathname: string): Promise<NextResponse> {
...
export async function proxyProgramFormConfigurationMutation(
  request: NextRequest,
  pathname: string,
