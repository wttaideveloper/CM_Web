...
export default function EventActionsMenu({ event, includeNavigation = true, onStatusSuccess, onDuplicateSuccess, onDeleteSuccess }: EventActionsMenuProps) {
  ...
  useEffect(() => {
    const closeOnOutsidePointer = (pointerEvent: MouseEvent) => { if (!menuRootRef.current?.contains(pointerEvent.target as Node)) setIsOpen(false); };
...
function openMenuWithKeyboard(event: React.KeyboardEvent<HTMLButtonElement>, menuRootRef: React.RefObject<HTMLDivElement | null>, setIsOpen: React.Dispatch<React.SetStateAction<boolean>>) { if (event.key !== "ArrowDown" && event.key !== "ArrowUp") return; setIsOpen(true); window.requestAnimationFrame(() => { const menuItems = menuRootRef.current?.querySelectorAll<HTMLElement>('[role="menuitem"]'); const target = event.key === "ArrowDown" ? menuItems?.[0] : menuItems?.[menuItems.length - 1]; target?.focus(); }); event.preventDefault(); }
