...
export function isCourseConfigurationId(value: string): boolean {
  return configurationIdPattern.test(value);
}

export function invalidCourseConfigurationIdResponse(): NextResponse {
  return NextResponse.json(
    { detail: "A valid course form configuration identifier is required." },
    { status: 400, headers: { "Cache-Control": "no-store" } },
  );
}

export async function proxyCourseFormConfigurationGet(pathname: string): Promise<NextResponse> {
...
export async function proxyCourseFormConfigurationMutation(
  request: NextRequest,
  pathname: string,
