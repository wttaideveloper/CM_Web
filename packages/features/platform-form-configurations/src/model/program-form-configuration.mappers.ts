...
import type { CreateProgramFormConfigurationRequest, ProgramFormConfiguration, ProgramFormConfigurationVersion, ProgramFormFieldInput, ProgramFormSectionInput, UpdateProgramFormConfigurationRequest } from "./program-form-configuration-api.types";
import { normalizeConfigurationOrder, normalizeOptionPositions } from "./form-configuration-ordering";

export type ProgramFormFieldCandidate = ProgramFormFieldInput;
export type ProgramFormSectionCandidate = ProgramFormSectionInput;
export type ProgramFormConfigurationCreateCandidate = CreateProgramFormConfigurationRequest;
export type ProgramFormConfigurationPatchCandidate = UpdateProgramFormConfigurationRequest;
...
const mapField = (field: ConfiguredField, includeServerId: boolean): ProgramFormFieldCandidate => ({ ...(includeServerId && field.serverId ? { id: field.serverId } : {}), source: field.source, core_key: field.coreKey, stable_key: field.stableKey, label: field.label, renderer: field.renderer, value_type: field.valueType, required: field.required, is_enabled: field.enabled, position: field.position, placeholder: field.placeholder || null, help_text: field.helpText || null, options: normalizeOptionPositions(field.options), validation: mapValidation(field.validation), ...(field.compositeConfig !== undefined ? { composite_config: field.compositeConfig } : {}) });
const mapSections = (configuration: FormConfiguration, includeServerIds: boolean): ProgramFormSectionCandidate[] => { const normalized = normalizeConfigurationOrder(configuration.sections, configuration.fields); return normalized.sections.map((section: FormSection) => ({ ...(includeServerIds && section.serverId ? { id: section.serverId } : {}), stable_key: section.stableKey, label: section.name, description: section.description || null, position: section.position, is_enabled: section.enabled, fields: normalized.fields.filter((field) => field.sectionLocalId === section.localId).map((field) => mapField(field, includeServerIds)) })); };
/** Pure future create payload candidate. It intentionally excludes server IDs and write transport. */
export function toProgramFormConfigurationCreateCandidate(configuration: FormConfiguration): ProgramFormConfigurationCreateCandidate { return { name: configuration.name, description: configuration.description || null, scope: configuration.scope, sections: mapSections(configuration, false) }; }
/** Pure future patch payload candidate. Scope and all write transport are intentionally excluded. */
export function toProgramFormConfigurationPatchCandidate(configuration: FormConfiguration): ProgramFormConfigurationPatchCandidate { return { name: configuration.name, description: configuration.description || null, scope: configuration.scope, sections: mapSections(configuration, true) }; }

/** Maps a persisted configuration or immutable version into the existing local builder model. */
export function toBuilderProgramFormConfiguration(configuration: ProgramFormConfiguration, version?: ProgramFormConfigurationVersion): FormConfiguration {
