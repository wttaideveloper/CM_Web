...
} from "../types/service-screen-config.types";
...
export default function ServiceEditScreen({
  ...
  const [providerMembers, setProviderMembers] = useState<ServiceProviderOption[]>([]);
  ...
  useEffect(() => {
    if (!enterpriseFilterId || !providerOptionsLoader) {
      return;
    }
    ...
    void providerOptionsLoader()
      .then((members) => {
        if (active) {
          setProviderMembers(members);
        }
      })
      .catch(() => {
        if (active) {
  ...
  }, [enterpriseFilterId, providerOptionsLoader]);
