...
function ProductCard({ product }: { product: EnterpriseProductSummary }) {
  const [hasImageError, setHasImageError] = useState(false);
  const imageSrc = product.productImages.trim() || "";
  const hasImage = Boolean(imageSrc) && !hasImageError;
  const productName = product.productName || "Unnamed Product";
  const productCategory = product.productCategory || "N/A";
  const productStatus = product.productStatus === false ? "Inactive" : "Active";
      ...
      <div className="p-4">
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          <p className="text-sm font-semibold text-[#06201c]">
            {formatCurrency(product.productPrice, product.currency)}
          </p>
        </div>
      </div>
    </article>
...
function ServiceCard({ service }: { service: EnterpriseServiceSummary }) {
  const serviceStatus = service.serviceStatus === false ? "Inactive" : "Active";
  const availabilityStatus =
    service.availabilityStatus === false ? "Unavailable" : "Available";
  const subtitle =
    service.serviceDescription.trim().slice(0, 84) || `${service.duration || 0} min`;
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            ...
            <p className="whitespace-nowrap text-sm font-bold text-[#06201c]">
              {formatCurrency(service.servicePrice, service.currency)}
            </p>
          </div>
...
export default function EnterpriseDetailsScreen({
  ...
  const [products, setProducts] = useState<EnterpriseProductSummary[]>([]);
  const [services, setServices] = useState<EnterpriseServiceSummary[]>([]);
  ...
  async function fetchEnterpriseProducts(enterpriseId: string | undefined) {
    if (!enterpriseId) {
      setProducts([]);
    ...
    try {
      ...
      const data = await enterpriseProductsLoader(enterpriseId);
      setProducts(data);
    } catch {
      setProducts([]);
    } finally {
  ...
  async function fetchEnterpriseServices(enterpriseId: string | undefined) {
    if (!enterpriseId) {
      setServices([]);
    ...
    try {
      ...
      const data = await enterpriseServicesLoader(enterpriseId);
      setServices(data);
    } catch {
      setServices([]);
    } finally {
  ...
  useEffect(() => {
    void fetchEnterpriseProducts(resolvedEnterpriseId);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [enterpriseProductsLoader, resolvedEnterpriseId]);

  useEffect(() => {
    void fetchEnterpriseServices(resolvedEnterpriseId);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [enterpriseServicesLoader, resolvedEnterpriseId]);
  ...
  const enterpriseServices = services;
  const stats = [
