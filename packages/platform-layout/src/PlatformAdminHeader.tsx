...
import type { PlatformAdminHeaderProps } from "./types";
  ...
  return (
    <Link href={homeHref} className="flex items-center gap-3 transition-opacity hover:opacity-90">
      <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#1f6a58] text-white">
        <svg aria-hidden="true" className="h-6 w-6" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M25 7C15.2 7.9 8.3 13.6 7.6 23.7C15.7 24.2 23.1 18.8 25 7Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
          <path d="M8.5 22.8C12 19.4 15.4 17.5 20 16.6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
        </svg>
      </div>
...
export function PlatformAdminHeader({
  ...
  return (
    <HeaderFrame
      ...
      left={(
        <>
          <button type="button" onClick={onMenuClick} className="flex h-9 w-9 items-center justify-center rounded-full text-[#52736a] hover:bg-[#f1f7f4] lg:hidden" aria-label="Open menu">
            <svg aria-hidden="true" className="h-5 w-5" viewBox="0 0 24 24" fill="none"><path d="M4 7h16M4 12h16M4 17h16" stroke="currentColor" strokeWidth="2" strokeLinecap="round" /></svg>
          </button>
      ...
      right={(
        <>
          ...
          {messagesHref ? (
            <Link href={messagesHref} className="relative flex h-9 w-9 items-center justify-center rounded-full text-[#52736a] hover:bg-[#f1f7f4]" aria-label="Messages" title="Messages">
              <MessageIcon />
            </Link>
          ) : null}
