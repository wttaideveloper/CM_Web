...
function PublishedEventCard({ event }: { event: EventItem }) {
  ...
  return <article className="overflow-hidden rounded-2xl border border-[#e1ebe6] bg-white shadow-sm transition-all duration-200 ease-out hover:-translate-y-1 hover:scale-[1.01] hover:shadow-lg"><div className="relative h-[180px] bg-gradient-to-br from-[#1f6a58] via-[#37836c] to-[#8ac7a7] p-5 text-white">{image && !imageFailed ? <img src={image} alt="" onError={() => setImageFailed(true)} className="absolute inset-0 h-full w-full object-cover" /> : <div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_18%,rgba(255,255,255,0.26)_0_1px,transparent_1px)] bg-[length:28px_28px]" />}<span className="absolute right-4 top-4 rounded-full bg-white/20 px-3 py-1 text-xs font-bold backdrop-blur">{temporalLabel(state)}</span><div className="absolute bottom-5 left-5 right-5"><p className="text-sm font-bold text-white/80">{dateLabel(event.start_date)}</p><h3 className="mt-1 text-xl font-bold leading-tight">{event.title}</h3></div></div><div className="space-y-3 p-5 text-sm text-[#52736a]">{organiser ? <p><span className="font-semibold text-[#31594d]">Organiser: </span>{organiser}</p> : null}<div><p className="font-medium text-[#31594d]">{venueLines[0]}</p>{venueLines.slice(1).map((line) => <p key={line} className="mt-0.5">{line}</p>)}</div>{capacity !== null ? <p><span className="font-semibold text-[#31594d]">Capacity: </span>{capacity}</p> : null}</div></article>;
}
