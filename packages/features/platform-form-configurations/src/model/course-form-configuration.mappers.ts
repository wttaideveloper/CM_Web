...
import type { CreateCourseFormConfigurationRequest, CourseFormConfiguration, CourseFormConfigurationVersion, CourseFormFieldInput, CourseFormSectionInput, UpdateCourseFormConfigurationRequest } from "./course-form-configuration-api.types";
import { normalizeConfigurationOrder, normalizeOptionPositions } from "./form-configuration-ordering";

export type CourseFormFieldCandidate = CourseFormFieldInput;
export type CourseFormSectionCandidate = CourseFormSectionInput;
export type CourseFormConfigurationCreateCandidate = CreateCourseFormConfigurationRequest;
export type CourseFormConfigurationPatchCandidate = UpdateCourseFormConfigurationRequest;
...
const mapField = (field: ConfiguredField, includeServerId: boolean): CourseFormFieldCandidate => ({ ...(includeServerId && field.serverId ? { id: field.serverId } : {}), source: field.source, core_key: field.coreKey, stable_key: field.stableKey, label: field.label, renderer: field.renderer, value_type: field.valueType, required: field.required, is_enabled: field.enabled, position: field.position, placeholder: field.placeholder || null, help_text: field.helpText || null, options: normalizeOptionPositions(field.options), validation: mapValidation(field.validation), ...(field.compositeConfig !== undefined ? { composite_config: field.compositeConfig } : {}) });
const mapSections = (configuration: FormConfiguration, includeServerIds: boolean): CourseFormSectionCandidate[] => { const normalized = normalizeConfigurationOrder(configuration.sections, configuration.fields); return normalized.sections.map((section: FormSection) => ({ ...(includeServerIds && section.serverId ? { id: section.serverId } : {}), stable_key: section.stableKey, label: section.name, description: section.description || null, position: section.position, is_enabled: section.enabled, fields: normalized.fields.filter((field) => field.sectionLocalId === section.localId).map((field) => mapField(field, includeServerIds)) })); };
/** Pure future create payload candidate. It intentionally excludes server IDs and write transport. */
export function toCourseFormConfigurationCreateCandidate(configuration: FormConfiguration): CourseFormConfigurationCreateCandidate { return { name: configuration.name, description: configuration.description || null, scope: configuration.scope, sections: mapSections(configuration, false) }; }
/** Pure future patch payload candidate. Scope and all write transport are intentionally excluded. */
export function toCourseFormConfigurationPatchCandidate(configuration: FormConfiguration): CourseFormConfigurationPatchCandidate { return { name: configuration.name, description: configuration.description || null, scope: configuration.scope, sections: mapSections(configuration, true) }; }

/** Maps a persisted configuration or immutable version into the existing local builder model. */
export function toBuilderCourseFormConfiguration(configuration: CourseFormConfiguration, version?: CourseFormConfigurationVersion): FormConfiguration {
