...
export default function EnterpriseAnalyticsScreen() {
  const [activeTab, setActiveTab] = useState<"overview" | "events" | "trainings">("overview");
  return (
    <div className="min-h-[calc(100vh-72px)] bg-[#f7fbf8] px-6 py-6">
      <div className="mx-auto w-full max-w-7xl space-y-5">
        ...
        <div role="tablist" aria-label="Analytics sections" className="border-b border-[#e1ebe6]">{(["overview", "events", "trainings"] as const).map((tab) => <button key={tab} id={`analytics-${tab}-tab`} type="button" role="tab" aria-selected={activeTab === tab} aria-controls={`analytics-${tab}-panel`} onClick={() => setActiveTab(tab)} className={`mr-6 border-b-2 px-1 pb-3 text-sm font-bold capitalize outline-none focus-visible:ring-2 focus-visible:ring-[#1f6a58] ${activeTab === tab ? "border-[#1f6a58] text-[#1f6a58]" : "border-transparent text-[#52736a]"}`}>{tab}</button>)}</div>
        {activeTab === "overview" ? <div id="analytics-overview-panel" role="tabpanel" aria-labelledby="analytics-overview-tab"><div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-4">{topMetrics.map((metric) => <MetricCard key={metric.label} {...metric} />)}</div><div className="mt-5 grid grid-cols-1 gap-5 xl:grid-cols-2"><RankingCard title="Top Services by Bookings" items={topServices} /><RankingCard title="Top Products by Revenue" items={topProducts} /></div></div> : activeTab === "events" ? <section id="analytics-events-panel" role="tabpanel" aria-labelledby="analytics-events-tab"><EventsAnalytics /></section> : <section id="analytics-trainings-panel" role="tabpanel" aria-labelledby="analytics-trainings-tab"><TrainingsAnalytics /></section>}
      </div>
    </div>
  );
}
