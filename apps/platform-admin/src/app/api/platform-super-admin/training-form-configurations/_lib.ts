...
export function isTrainingConfigurationId(value: string): boolean {
  return configurationIdPattern.test(value);
}

export function invalidTrainingConfigurationIdResponse(): NextResponse {
  return NextResponse.json(
    { detail: "A valid training form configuration identifier is required." },
    { status: 400, headers: { "Cache-Control": "no-store" } },
  );
}

export async function proxyTrainingFormConfigurationGet(pathname: string): Promise<NextResponse> {
...
export async function proxyTrainingFormConfigurationMutation(
  request: NextRequest,
  pathname: string,
